        ...
        <div className="animate-in">
          <div style={{ borderRadius:'var(--radius-xl)', background:'var(--navy)', padding:'32px', marginBottom:16, position:'relative', overflow:'hidden' }}>
            ...
            <p style={{ fontSize:15, color:'rgba(255,255,255,0.7)', maxWidth:480, lineHeight:1.6 }}>
              Connect your knowledge base in 2 minutes and we'll tell you exactly what's hurting your customer experience — and what to fix first.
            </p>
          </div>
          <div className='onboard-grid' style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12, marginBottom:16 }}>
            {[
              { step:1, icon:<Plug size={16} style={{ color:'var(--navy)' }} />, title:'Connect your KB', desc:'Connect Zendesk® or HelpScout. Read-only — we never touch your articles.', action:true },
              ...
              <div key={step} className="card" style={{ padding:'20px', borderTop:`3px solid ${action ? 'var(--navy)' : 'var(--border)'}` }}>
                ...
                {action && <Link to="/connector" className="btn btn-primary btn-sm" style={{ width:'100%', justifyContent:'center' }}><Plug size={12} /> Connect your KB →</Link>}
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 16px', borderRadius:10, marginBottom:14,
              ...
              <div>
                ...
                <p style={{ fontSize:11, color:'var(--text-3)', margin:0 }}>
                  {profile.scans_remaining === 0
                    ...
                    : profile.scans_remaining <= 2 ? `🔥 As a thank you for being a Scan Pack customer — $50 off Annual Pro, just for you` : 'Scan Pack · credits never expire · upgrade anytime for unlimited scans'}
                </p>
              </div>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              ...
              {profile.scans_remaining > 0 && profile.scans_remaining <= 2 ? (
                <button onClick={() => navigate('/upgrade?coupon=UPGRADE50')} className="btn btn-sm"
                  style={{ background:'linear-gradient(135deg, #FF6B35, #FF8C00)', color:'white', fontWeight:800, border:'none', boxShadow:'0 2px 8px rgba(255,107,53,0.4)', letterSpacing:'-0.2px' }}>
                  🔥 Claim $50 off
                </button>
              ) : (

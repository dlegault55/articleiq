// ─── ArticleIQ — Email notifications via Resend ───────────────
// Called client-side after scan completes.
...
export const sendScanCompleteEmail = async ({ to, firstName, scanId, articles, critical, warning, scanDate }) => {
  if (!RESEND_KEY || RESEND_KEY === 'REPLACE_WITH_YOUR_RESEND_KEY') {
    console.log('[Email] Resend key not configured — skipping email')
    return
  }

  const reportUrl = `${window.location.origin}/scanner/results/${scanId}`
...
<body style="margin:0;padding:0;background:#F5F5F4;font-family:Inter,sans-serif">
  <div style="max-width:560px;margin:32px auto;background:#fff;border-radius:12px;border:1px solid #E2E2DE;overflow:hidden">
    ...
    <div style="padding:16px 32px;border-top:1px solid #E2E2DE;background:#F5F5F4">
      <p style="font-size:12px;color:#9B9B96;margin:0">ArticleIQ · <a href="${window.location.origin}" style="color:#107C10;text-decoration:none">articleiq.vercel.app</a></p>
    </div>
  </div>
</body>
</html>`
